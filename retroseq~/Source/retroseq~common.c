...
void *common_new(t_retroseq *x, short argc, t_atom *argv)
{
    ...
    if (x->waveform == gensym("sine")) {
        x->waveform = gensym("");
        retroseq_build_sine(x);
    } else if (x->waveform == gensym("triangle")) {
        x->waveform = gensym("");
        retroseq_build_triangle(x);
    } else if (x->waveform == gensym("sawtooth")) {
        x->waveform = gensym("");
        retroseq_build_sawtooth(x);
    } else if (x->waveform == gensym("square")) {
        x->waveform = gensym("");
        retroseq_build_square(x);
    } else if (x->waveform == gensym("pulse")) {
        x->waveform = gensym("");
        retroseq_build_pulse(x);
    } else {
        x->waveform = gensym("");
        retroseq_build_sine(x);
        
        error("retroseq~ • Invalid argument: Waveform set to %s", x->waveform->s_name);
    }
