...
void *new_memory(long nbytes)
{
    ...
    if (pointer == NULL) {
        error("bed • Cannot allocate memory for this object");
        return NULL;
    }
    return pointer;
}
...
void *common_new(t_bed *x, short argc, t_atom *argv)
{
    ...
    /* Initialize clocks */
    x->bang_clock = clock_new(x, (method)bed_send_bang);
    x->adsr_clock = clock_new(x, (method)bed_send_adsr);
#elif TARGET_IS_PD
    /* Create inlets */
    //nothing
    ...
    /* Initialize clocks */
    x->bang_clock = clock_new(x, (t_method)bed_send_bang);
    x->adsr_clock = clock_new(x, (t_method)bed_send_adsr);
#endif
...
{
    if (argc < 2) {
        error("bed • The sequence must have at least two members");
        return;
    }
    if (argc % 2) {
        error("bed • The sequence must have odd number of members");
        return;
    }
...
void bed_permute(float *sequence, int length)
{
    while (length > 0) {
...
{
    bed_permute(x->note_sequence, x->note_sequence_length);
...
{
    bed_permute(x->duration_sequence, x->duration_sequence_length);
...
void bed_set_tempo(t_bed *x, t_symbol *msg, short argc, t_atom *argv)
{
    ...
    if (new_tempo_bpm <= 0) {
        error("bed • Tempo must be greater than zero");
        return;
    }
...
{
    if (argc != 4) {
        error("bed • The envelope must have four members");
        return;
    }
...
void bed_dsp(t_bed *x, t_signal **sp, short *count)
{
    ...
    dsp_add(bed_perform, NEXT-1, x,
            sp[0]->s_vec,
            sp[0]->s_n);

    /* Print message to Max window */
    post("bed • Executing 32-bit perform routine");
}
